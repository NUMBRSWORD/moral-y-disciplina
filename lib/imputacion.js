...
export function buildDescripcionHecho(nota) {
  ...
  const sufijo = (numero) => `NOTA INFORMATIVA N° ${numero}`;
  // Si se conoce el puesto asignado ese día (extraído del rol de servicio), se
  // precisa en la redacción; si no, queda la fórmula genérica de siempre.
  const enElServicio = nota.puesto_rol ? ` en el servicio de ${String(nota.puesto_rol).trim()}` : "";

  return (
    `en circunstancias que el investigado se encontraba nombrado según el rol de servicio de la unidad consignada en el expediente${enElServicio}, ` +
...
export function construirDatosImputacion(nota, efectivos) {
  ...
  return {
    ...
    unidad_investigado: nota.unidad_investigado || "[UNIDAD POR CONFIRMAR]",
    ...
    fecha_larga: fechaLarga(hoyLima()),
    fecha_corta: fechaCorta(hoyLima()),
...
export async function generarImputacionDocx(nota, efectivos) {
  ...
  const nombreArchivo = `IMPUTACION LEVE - ${(nota.grado || "").trim()} ${nombreCompletoVisible(nota.apellidos, nota.nombres)}.docx`.replace(/\s+/g, " ").trim();
