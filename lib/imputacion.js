...
// libre, tal como quedó registrado en la nota, p.ej. "TNTE. RIOS PAREDES
// Diego" o "S2 PNP GOMEZ PRADO").
const GRADOS = new Set([
  "GRAL", "TGRAL", "CGRAL", "CRNL", "CMDTE", "CMDT", "MY", "MAY", "CAP",
  "TNTE", "TTE", "ALF", "SO", "SOB", "SOT", "SO1", "SO2", "SO3",
  "S1", "S2", "S3", "ST1", "ST2", "ST3", "PNP", "EST", "CADETE", "SUBOF",
]);
...
// espacio detrás (p.ej. "VARGAS SOTO,MARIO ENRIQUE"); esto lo deja legible
// ("VARGAS SOTO, MARIO ENRIQUE") para usarlo en el cuerpo del documento y el
...
function normalizarTexto(s) {
  return (s || "")
    ...
    // detrás (p.ej. "VARGAS SOTO,MARIO ENRIQUE"); quitar la coma sin
...
// siempre "JEFE DE PERMANENCIA" (así se confirmó para el caso de prueba);
// si en el futuro varía según el oficial, este es el único lugar a ajustar.
const CARGO_SELLO_POR_DEFECTO = "JEFE DE PERMANENCIA";
...
export function construirDatosImputacion(nota, efectivos) {
  ...
  const investigadoCompleto = `${nota.grado || ""} PNP ${nota.apellidos || ""}, ${nota.nombres || ""}`.replace(/\s+/g, " ").trim();
  const gradoOficialSinPunto = (oficial.grado || "").replace(/\.$/, "").trim();
  ...
  return {
    ...
    superior_completo: `${oficial.grado || ""} PNP ${limpiarNombreVisible(oficial.apellidos_nombres)}.`.replace(/\s+/g, " ").trim(),
    ...
    oficial_grado_seal: `${gradoOficialSinPunto} PNP`,
