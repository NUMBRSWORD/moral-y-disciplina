import { fechaLima, hoyLima, siguienteDiaHabil } from "./fechas.js";
...
// los apellidos en MAYÚSCULAS (p.ej. "Manuel Angelo REYES MEDINA"). Recibe
...
// Ahora salta también los feriados y días no laborables (TUO Ley 27444, art. 134.1):
// ver lib/fechas.js.
export { siguienteDiaHabil };

export function fechaLimiteDescargo(nota) {
  if (!nota.imputacion_generada_at) return null;
  return siguienteDiaHabil(fechaLima(nota.imputacion_generada_at));
}

export function plazoDescargoVencido(nota, hoyISO) {
  if (!nota.imputacion_generada_at) return false;
  const hoy = hoyISO || hoyLima();
  const fechaNotif = fechaLima(nota.imputacion_generada_at);
