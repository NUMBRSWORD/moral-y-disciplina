...
import { hoyLima, horaLima } from "./fechas.js";
...
import { plazoDescargoVencido, fechaLimiteDescargo, tituloPorPalabra, nombreCompletoVisible } from "./utils.js";
...
export function construirDatosActaNoDescargo(nota, efectivos) {
  ...
  return {
    fecha_larga: fechaLarga(hoyLima(ahora)),
    hora_apertura: horaLima(ahora),
    hora_cierre: horaLima(cierre),
    ...
    investigado_apellidos: (nota.apellidos || "").trim().toUpperCase(),
    investigado_nombres: tituloPorPalabra(nota.nombres),
...
export async function generarActaNoDescargoDocx(nota, efectivos) {
  ...
  const nombreArchivo = `ACTA NO DESCARGO - ${(nota.grado || "").trim()} ${nombreCompletoVisible(nota.apellidos, nota.nombres)}.docx`.replace(/\s+/g, " ").trim();
