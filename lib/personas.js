...
// Valor especial del selector de período: suma las faltas de varios meses (desde
// la fecha `desde`, si se da) para que los retrasos de meses distintos se acumulen.
export const PERIODO_ACUMULADO = "acumulado";

// Quiénes faltaron en un mes ("2026-09") o, con PERIODO_ACUMULADO, desde `desde`
// ("2026-09-01") en adelante: una fila por efectivo, con cuántas
...
export function resumenDelMes(notas, ym, ahora = new Date(), desde = null) {
  const delMes = (notas || []).filter((n) => {
    const f = String(n.fecha_falta ?? "");
    if (ym === PERIODO_ACUMULADO) return !!f && (!desde || f >= desde);
    return f.slice(0, 7) === ym;
  });
...
// Igual que formatearDuracion pero SIN minutos: solo días y horas completas (los
// minutos sueltos no se muestran ni se redondean). Es lo que usa el Panel, que
// da un cálculo aproximado; los días y minutos exactos se cuentan en el informe.
export function formatearDuracionHoras(horas) {
  if (!Number.isFinite(horas) || horas <= 0) return "0h";
  const totalH = Math.floor(Math.round(horas * 60) / 60);
  const dias = Math.floor(totalH / 24);
  return dias ? `${dias}d ${totalH % 24}h` : `${totalH}h`;
}
