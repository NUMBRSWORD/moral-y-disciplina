...
// `entrante` es la fila leída de un PDF: { apellidos, nombres,
// numero_nota_falta, referencia } -- `referencia` es el N.º de la nota a la que
// el PDF dice responder ("REF."), si lo trae. `notas` son los expedientes ya
// guardados y `lote` las demás filas que se están subiendo junto con esta
// (puede incluir a `entrante` misma; se ignora).
//
...
//  - "continuacion": su REF. apunta a la falta de esta misma persona, ya sea en
//    la base o en el mismo lote (aunque el PDF madre venga después en la lista).
...
export function clasificarNotaEntrante(entrante, notas, lote = []) {
  ...
  if (!numero && !referencia) return null;
