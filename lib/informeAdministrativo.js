...
// (copia con placeholders {tag} del formato real aprobado por el usuario; el ejemplo
// con datos reales se guarda fuera del repo, que es público).
...
//   Especial de El Peruano del 12-nov-2025 y vuelto a cotejar el 21-sep-2026
//   contra los PDF oficiales del D.S. 016-2025-IN (G39 y MG32: texto y
//   sanción idénticos). La sanción de MG32 es pase a retiro: aun así conviene
//   que el responsable lo compare con su propia copia antes de firmar.
...
// El tope del guard (400 días, ~13 meses) es solo para no colgarse ante
// fechas mal formadas -- una ausencia MUY GRAVE "en curso" (sin
// reincorporación) puede legítimamente seguir abierta varios meses mientras
// se resuelve el pase a retiro, así que no puede ser un número chico.
export function diasDeAusencia(fechaFalta, fechaReincorporacion) {
  const dias = [];
  let cursor = fechaFalta;
  let guard = 0;
  while (cursor <= fechaReincorporacion && guard < 400) {
    dias.push(cursor);
...
export function construirDatosInformeAdministrativo(nota, efectivos, rolesServicio, firmantes) {
  ...
  const fechaCierre = enCurso ? hoyLima() : nota.fecha_reincorporacion;
  ...
  const bloquesDias = fechasAusencia.map((fecha, i) => {
    ...
    if (esPrimero) {
      return `${letra}. Día ${fechaCompacta(fecha)} (primer día de inasistencia): Al pasarse lista al personal policial de servicio a las ${horaFalta} horas${nota.oficial_constato ? ` por parte del ${nota.oficial_constato}` : ""}, se constató la ausencia física del ${investigadoCompleto}, y se formuló la Nota Informativa N° ${nota.numero_nota_falta}.`;
    }
    if (esUltimo && !enCurso) {
      return `${letra}. Día ${fechaCompacta(fecha)} (reincorporación): A las ${horaReinc} horas, el ${investigadoCompleto} se reincorporó físicamente a su unidad de servicio. El hecho fue comunicado mediante Nota Informativa N° ${nota.numero_nota_reincorporacion}.`;
    }
    ...
    if (seguimiento) {
      return `${encabezado}: Al pasarse revista${seguimiento.oficial_constato ? ` por parte del ${seguimiento.oficial_constato}` : ""}, se constató que el administrado continuaba falto al servicio, y se formuló la Nota Informativa N° ${seguimiento.numero_nota}.`;
    }
    ...
    return `${encabezado}: El administrado continuó sin presentarse a su unidad de servicio, ${detalleRol}.`;
  });
  ...
  const conclusiones = [
    ...
    enCurso
      ? `2. A la fecha de elaboración del presente informe, el administrado NO se ha reincorporado a su unidad de servicio, manteniéndose la inasistencia de manera ininterrumpida.`
      : `2. El administrado se reincorporó físicamente a su unidad de servicio a las ${horaReinc} horas del ${fechaReincCompacta}, conforme a la Nota Informativa N° ${nota.numero_nota_reincorporacion}.`,
  ...
  const anexos = [
    `1. Rol(es) de servicio de la unidad consignada en el expediente correspondientes a los días ${fechasAusencia.map(fechaCompacta).join(", ")}.`,
  ...
  return {
    ...
    ref_texto: enCurso
      ? `Ley N° 30714, Ley que regula el Régimen Disciplinario de la Policía Nacional del Perú; Reglamento aprobado por Decreto Supremo N° 003-2020-IN, modificado por Decreto Supremo N° 016-2025-IN; Nota Informativa N° ${nota.numero_nota_falta}.`
      : `Ley N° 30714, Ley que regula el Régimen Disciplinario de la Policía Nacional del Perú; Reglamento aprobado por Decreto Supremo N° 003-2020-IN, modificado por Decreto Supremo N° 016-2025-IN; Notas Informativas N.os ${nota.numero_nota_falta} y ${nota.numero_nota_reincorporacion}.`,
    ...
    antecedentes_p3: enCurso
      ? `A la fecha de elaboración del presente informe, el administrado no ha reportado su reincorporación a su unidad de servicio ni ha justificado documentalmente su inasistencia.`
      : `Mediante Nota Informativa N° ${nota.numero_nota_reincorporacion} se dejó constancia de que el administrado se reincorporó físicamente a su unidad de servicio a las ${horaReinc} horas del ${fechaReincCompacta}.`,
    dias,
    ...
    recomendaciones,
    anexos,
    fecha_larga_cierre: `${nota.lugar || "[LUGAR POR CONFIRMAR]"} ${fechaLarga(hoyLima())}`,
