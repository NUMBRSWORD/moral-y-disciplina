...
export function construirDatosInformeAdministrativo(nota, efectivos, rolesServicio, firmantes) {
  ...
  const bloquesDias = dias.map((fecha, i) => {
    ...
    const detalleRol = rol
      ...
      : `sin que se ubique en el sistema un rol de servicio guardado para esa fecha que lo acredite — verificar manualmente antes de remitir`;
  ...
  const rolesFaltantes = diasIntermedios.filter((fecha) => !(rolesServicio || []).some((r) => r.fecha === fecha));
  ...
  return {
    ...
    servicio_roles_texto: rolesFaltantes.length
      ? `registrado como falto al servicio; no se ubicó en el sistema el rol guardado de ${rolesFaltantes.map(fechaCompacta).join(", ")} — verificar antes de remitir`
      : "registrado como falto al servicio en los roles examinados",
    indicios_lista: [
