...
const TERCIOS_POR_CODIGO = {
  L21: [
    { value: "amonestacion", label: "Amonestación (tercio inferior)", fragmento: "AMONESTACIÓN" },
    { value: "2", label: "Dos (02) días de Sanción Simple (tercio medio)", fragmento: "DOS (02) días de Sanción Simple" },
    { value: "4", label: "Cuatro (04) días de Sanción Simple (tercio superior)", fragmento: "CUATRO (04) días de Sanción Simple" },
  ],
  L24: [
    { value: "8", label: "Ocho (08) días de Sanción Simple", fragmento: "OCHO (08) días de Sanción Simple" },
    { value: "10", label: "Diez (10) días de Sanción Simple", fragmento: "DIEZ (10) días de Sanción Simple" },
  ],
};
...
function buildCasoConcreto(nota) {
...
export function construirDatosOrdenSancion(nota, efectivos, seleccion) {
  ...
  const descargoTexto = (seleccion.descargoTexto || "").trim() ||
    "El investigado no presentó descargo dentro del plazo establecido.";

  const hechoCompleto = `"${infraccionSinPunto}"; CASO CONCRETO: ${buildCasoConcreto(nota)}`;

  const decisionTexto =
    `Se resuelve SANCIONAR al ${investigadoCompleto}, CIP N° ${investigadoEf.cip}, perteneciente a la comisaria PNP Ventanilla, ` +
