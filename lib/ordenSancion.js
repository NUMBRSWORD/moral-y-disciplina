...
// usuario; el ejemplo con datos reales se guarda fuera del repo, que es público).
...
export function buildCasoConcreto(nota) {
  ...
  const enElServicio = nota.puesto_rol ? ` en el servicio de ${String(nota.puesto_rol).trim()}` : "";

  return (
    `El investigado se encontraba nombrado según el rol de servicio de la unidad consignada en el expediente${enElServicio} para el turno de las ${horaFalta} horas del ${fechaFaltaCorta}; ` +
...
export function puedeGenerarOrdenSancion(nota, efectivos) {
  ...
  // El CIP del investigado se guarda en la propia nota (investigado_cip); si es
  // una nota antigua sin ese dato, se recurre al padrón (solo lo tiene el admin).
  if (!nota.investigado_cip && !buscarInvestigado(nota.apellidos, nota.nombres, efectivos)) return false;
  return true;
}
...
export function construirDatosOrdenSancion(nota, efectivos, seleccion) {
  ...
  const investigadoCip = nota.investigado_cip || buscarInvestigado(nota.apellidos, nota.nombres, efectivos)?.cip;
  if (!investigadoCip) {
    throw new Error(`No se pudo determinar el CIP de ${nota.apellidos || ""} ${nota.nombres || ""}. Verifique que esté registrado en Efectivos (lo hace el administrador).`);
  }
  ...
  const descargoTexto = normalizarMencionInvestigado(descargoOriginal, nota.apellidos, nota.nombres);
  const analisisTexto = normalizarMencionInvestigado(seleccion.analisisTexto.trim(), nota.apellidos, nota.nombres);

  // La plantilla ya muestra el código y el texto normativo de la infracción
  // en `codigo_texto`. Aquí va exclusivamente el hecho concreto, sin repetir
  // la infracción (antes se anteponía `"{infracción}"; CASO CONCRETO: ...`).
  const hechoCompleto = buildCasoConcreto(nota);

  const decisionTexto =
    `Se resuelve SANCIONAR al ${investigadoCompleto}, CIP N° ${investigadoCip}, perteneciente a la unidad consignada en el expediente, ` +
  ...
  return {
    ...
    fecha_larga_punto: `${nota.lugar || "[LUGAR POR CONFIRMAR]"}, ${fechaLarga(hoyISO)}.`,
...
export async function generarOrdenSancionDocx(nota, efectivos, seleccion) {
  ...
  const nombreArchivo = `ORDEN DE SANCION - ${(nota.grado || "").trim()} ${nombreCompletoVisible(nota.apellidos, nota.nombres)}.docx`.replace(/\s+/g, " ").trim();
