...
import { agruparPersonas, buscarPersonas, horasDelCaso, formatearDuracion, resumenPersona, resumenDelMes, PERIODO_ACUMULADO, formatearDuracionHoras } from "./personas.js";
...
describe("resumenDelMes", () => {
  ...
  test("el acumulado suma a través de los meses", () => {
    const todo = resumenDelMes(NOTAS, PERIODO_ACUMULADO, AHORA);
    assert.equal(todo.total, 6); // 5 de septiembre + 1 de agosto
    assert.equal(todo.efectivos.length, 4);
    const juan = todo.efectivos[0];
    assert.equal(juan.apellidos, "LOPEZ GARAY");
    assert.equal(juan.faltas, 3);
    assert.equal(juan.horasTotales, 107); // 11h (agosto) + 48h + 48h en curso
    assert.equal(juan.duracion, "4d 11h");
    assert.deepEqual(juan.fechas, ["2026-08-10", "2026-09-01", "2026-09-15"]);
  });
  test("el acumulado no cuenta expedientes sin fecha de falta", () => {
    const r = resumenDelMes([n({ fecha_falta: null }), n({ fecha_falta: "2026-09-02" })], PERIODO_ACUMULADO, AHORA);
    assert.equal(r.total, 1);
  });
  test("el acumulado puede empezar en una fecha: lo anterior no cuenta", () => {
    const r = resumenDelMes(NOTAS, PERIODO_ACUMULADO, AHORA, "2026-09-01");
    assert.equal(r.total, 5); // sin la falta de agosto
    const juan = r.efectivos[0];
    assert.equal(juan.faltas, 2);
    assert.equal(juan.horasTotales, 96); // 48h + 48h en curso, sin las 11h de agosto
    assert.deepEqual(juan.fechas, ["2026-09-01", "2026-09-15"]);
  });
...
describe("formatearDuracionHoras (Panel: sin minutos)", () => {
  test("solo días y horas completas; los minutos no se muestran ni redondean", () => {
    assert.equal(formatearDuracionHoras(23.5), "23h");        // 23:30 -> 23h
    assert.equal(formatearDuracionHoras(47.25), "1d 23h");    // 47:15 -> 1d 23h
    assert.equal(formatearDuracionHoras(23 + 59 / 60), "23h"); // 23:59 sigue sin ser un día
    assert.equal(formatearDuracionHoras(24), "1d 0h");
    assert.equal(formatearDuracionHoras(27.5), "1d 3h");
    assert.equal(formatearDuracionHoras(72), "3d 0h");
  });
  test("menos de una hora o dato inválido: 0h", () => {
    assert.equal(formatearDuracionHoras(0.75), "0h");
    assert.equal(formatearDuracionHoras(0), "0h");
    assert.equal(formatearDuracionHoras(NaN), "0h");
  });
});
...
describe("retrasos acumulados", () => {
  ...
  // La suma es para el descuento. NO reclasifica la infracción: cada falta
  // conserva su código (dos faltas de 23:30 h siguen siendo dos L21, no una G39).
  test("dos faltas de 23:30 h suman 47:00 h = 1d 23h de tiempo no laborado", () => {
