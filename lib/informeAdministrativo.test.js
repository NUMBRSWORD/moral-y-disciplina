...
describe("puedeGenerarInformeAdministrativo", () => {
  ...
  test("false para MG32: el texto legal todavía no está cargado", () => {
    assert.equal(puedeGenerarInformeAdministrativo({ ...NOTA_G39, codigo_infraccion: "MG32" }, []), false);
  });
});
...
describe("construirDatosInformeAdministrativo", () => {
  ...
  test("el día intermedio cita la nota de \"Continúan faltos\" (seguimiento_faltas) en vez del rol, cuando existe", () => {
    ...
    assert.match(d.descripcion_dias, /CAP\. PNP QUISPE SANTOS Paolo Cesar/);
  ...
  test("lanza error claro para MG32 en vez de inventar el texto legal", () => {
    assert.throws(
      () => construirDatosInformeAdministrativo({ ...NOTA_G39, codigo_infraccion: "MG32" }, [], [], FIRMANTES),
      /No hay texto legal verificado.*MG32/
    );
  });
