...
describe("limpiarNombreVisible", () => {
  test("apellidos en mayúsculas, nombres en formato título (con coma)", () => {
    assert.equal(limpiarNombreVisible("VARGAS SOTO,MARIO ENRIQUE"), "VARGAS SOTO, Mario Enrique");
  });
  test("apellidos en mayúsculas, nombres en formato título (sin coma)", () => {
    assert.equal(limpiarNombreVisible("RIOS PAREDES DIEGO"), "RIOS PAREDES Diego");
  });
