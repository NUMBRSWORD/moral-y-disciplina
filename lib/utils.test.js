...
describe("tokens", () => {
  test("quita tildes, puntuación y mayúsculas todo", () => {
    assert.deepEqual(tokens("TNTE. Ríos Parédes Diego"), ["TNTE", "RIOS", "PAREDES", "DIEGO"]);
  });
...
describe("sugerirCodigoInfraccion", () => {
  ...
  test("límites L21/L24/G39/MG32", () => {
    ...
    assert.equal(sugerirCodigoInfraccion(23.99), "L21"); // 23:59 h: mientras no se cumplan las 24:00 h es retraso
...
describe("plazo de descargo en hora de Lima y con feriados", () => {
  // Notificada a las 20:30 de Lima del 9-sep = 01:30 UTC del 10-sep
  const nota = (ts) => ({ imputacion_generada_at: ts });
  test("una notificación de noche cuenta en el día de Lima, no en el de UTC", () => {
    assert.equal(fechaLimiteDescargo(nota("2026-09-10T01:30:00+00:00")), "2026-09-10");
  });
  test("no se da por vencido durante el último día del plazo, ni de noche", () => {
    const n = nota("2026-09-09T15:00:00+00:00"); // 9-sep 10:00 Lima -> límite 10-sep
    assert.equal(plazoDescargoVencido(n, "2026-09-10"), false);
    assert.equal(plazoDescargoVencido(n, "2026-09-11"), true);
  });
  test("la víspera de un feriado el plazo pasa al siguiente día hábil (Angamos, 8-oct)", () => {
    const n = nota("2026-10-07T15:00:00+00:00"); // miércoles 7-oct 10:00 Lima
    assert.equal(fechaLimiteDescargo(n), "2026-10-09");
    assert.equal(plazoDescargoVencido(n, "2026-10-09"), false);
    assert.equal(plazoDescargoVencido(n, "2026-10-10"), true);
  });
});
