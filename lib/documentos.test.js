...
import { construirDatosArchivoLeve, puedeGenerarArchivoLeve } from "./archivoLeve.js";
...
describe("construirDatosImputacion", () => {
  ...
  test("incluye el puesto del rol en la descripción del hecho cuando existe", () => {
    const sin = construirDatosImputacion(NOTA, EFECTIVOS);
    assert.doesNotMatch(sin.descripcion_hecho, /en el servicio de/);
    const con = construirDatosImputacion({ ...NOTA, puesto_rol: "SECCION DE FAMILIA" }, EFECTIVOS);
    assert.match(con.descripcion_hecho, /rol de servicio de la unidad consignada en el expediente en el servicio de SECCION DE FAMILIA,/);
    assert.doesNotMatch(con.descripcion_hecho, /Ventanilla|COMOPPOL-PNP/);
  });
});
...
describe("construirDatosOrdenSancion", () => {
  ...
  test("si la nota trae puesto_rol, la narrativa lo precisa; si no, queda genérica", () => {
    const gen = construirDatosOrdenSancion(NOTA, EFECTIVOS, seleccion);
    assert.match(gen.hecho_completo, /según el rol de servicio de la unidad consignada en el expediente para el turno/);
    assert.doesNotMatch(gen.hecho_completo, /Ventanilla|COMOPPOL-PNP/);
    const conPuesto = construirDatosOrdenSancion({ ...NOTA, puesto_rol: "PATRULLAJE A PIE - SECTOR 3" }, EFECTIVOS, seleccion);
    assert.match(conPuesto.hecho_completo, /en el servicio de PATRULLAJE A PIE - SECTOR 3 para el turno/);
  });
...
describe("construirDatosArchivoLeve / puedeGenerarArchivoLeve", () => {
  const seleccion = { motivoTexto: "no se ubica en el sistema una nota que acredite el hecho imputado con certeza.", resolucionNumero: "N° 001-2026-COMVENT-A" };

  test("true cuando hay descargo y no hay Orden de Sanción generada", () => {
    assert.equal(puedeGenerarArchivoLeve(NOTA, EFECTIVOS), true);
  });
  test("false sin descargo -- es la condición explícita del usuario, no solo cualquier caso leve", () => {
    assert.equal(puedeGenerarArchivoLeve({ ...NOTA, fecha_descargo: null }, EFECTIVOS), false);
  });
  test("false si ya hay una Orden de Sanción generada -- mutuamente excluyente", () => {
    assert.equal(puedeGenerarArchivoLeve({ ...NOTA, orden_sancion_generada_at: "2026-08-15T00:00:00Z" }, EFECTIVOS), false);
  });
  test("el hecho es el mismo que arma buildCasoConcreto -- no se inventa un segundo relato", () => {
    const d = construirDatosArchivoLeve(NOTA, EFECTIVOS, seleccion);
    assert.equal(d.hecho_completo, buildCasoConcreto(NOTA));
  });
  test("cita el código imputado con guion, igual que la Orden de Sanción", () => {
    const d = construirDatosArchivoLeve(NOTA, EFECTIVOS, seleccion);
    assert.equal(d.codigo_imputado, "L-21");
  });
  test("normaliza las menciones al investigado dentro del motivo", () => {
    const d = construirDatosArchivoLeve({ ...NOTA }, EFECTIVOS, { ...seleccion, motivoTexto: "MENDOZA RAMOS Carlos no presentó ausencia injustificada según el descargo." });
    assert.match(d.motivo_archivo, /Carlos MENDOZA RAMOS/);
    assert.doesNotMatch(d.motivo_archivo, /MENDOZA RAMOS Carlos/);
  });
  test("la firma es una sola persona (el sancionador), igual que la Orden de Sanción", () => {
    const d = construirDatosArchivoLeve(NOTA, EFECTIVOS, seleccion);
    assert.equal(d.signer_nombre, "Diego Martin RIOS PAREDES");
  });
  test("lanza error si falta el motivo del archivo", () => {
    assert.throws(
      () => construirDatosArchivoLeve(NOTA, EFECTIVOS, { ...seleccion, motivoTexto: "  " }),
      /motivo/i,
    );
  });
  test("lanza error si falta el N.º de Resolución", () => {
    assert.throws(
      () => construirDatosArchivoLeve(NOTA, EFECTIVOS, { ...seleccion, resolucionNumero: "" }),
      /Resolución/,
    );
  });
  test("lanza error si ya hay una Orden de Sanción generada", () => {
    assert.throws(
      () => construirDatosArchivoLeve({ ...NOTA, orden_sancion_generada_at: "2026-08-15T00:00:00Z" }, EFECTIVOS, seleccion),
      /Orden de Sanción/,
    );
  });
  test("lanza error si no hay descargo", () => {
    assert.throws(
      () => construirDatosArchivoLeve({ ...NOTA, fecha_descargo: null }, EFECTIVOS, seleccion),
      /descargo/i,
    );
  });
});
