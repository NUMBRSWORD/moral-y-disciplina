...
describe("construirDatosImputacion", () => {
  ...
  test("incluye el puesto del rol en la descripción del hecho cuando existe", () => {
    ...
    assert.match(con.descripcion_hecho, /rol de servicio de la Comisaría PNP Ventanilla en el servicio de SECCION DE FAMILIA,/);
  });
});
...
describe("construirDatosOrdenSancion", () => {
  ...
  test("si la nota trae puesto_rol, la narrativa lo precisa; si no, queda genérica", () => {
    ...
    assert.match(gen.hecho_completo, /según el rol de servicio de la Comisaría PNP Ventanilla para el turno/);
  ...
  test("usa investigado_cip de la nota sin necesitar al investigado en el padrón", () => {
    ...
    const soloOficial = EFECTIVOS.filter((e) => e.cip !== "32138999");
...
describe("construirDatosArchivoLeve / puedeGenerarArchivoLeve", () => {
  ...
  test("normaliza las menciones al investigado dentro del motivo", () => {
    const d = construirDatosArchivoLeve({ ...NOTA }, EFECTIVOS, { ...seleccion, motivoTexto: "SALGADO CHOQUE Freddy no presentó ausencia injustificada según el descargo." });
    assert.match(d.motivo_archivo, /Freddy SALGADO CHOQUE/);
    assert.doesNotMatch(d.motivo_archivo, /SALGADO CHOQUE Freddy/);
  });
